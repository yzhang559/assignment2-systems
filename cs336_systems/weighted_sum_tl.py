...
# Final API: works like torch.nn.functional functions
f_weightedsum = WeightedSumFunc.apply

if __name__ == '__main__':
    from cs336_systems.example import f_weightedsum

    x = torch.randn(64, 128, device='cuda', requires_grad=True)
    w = torch.randn(128, device='cuda', requires_grad=True)

    y = f_weightedsum(x, w)  # Forward pass
    # y.grad_fn shows <WeightedSumFuncBackward>

    loss = y.sum()
    loss.backward()  # Backward pass uses your Triton kernel

    print(x.grad.shape)  # [64, 128]
    print(w.grad.shape)  # [128]
